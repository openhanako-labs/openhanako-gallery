...
const TOOLS = [
  ...
  serviceTool({
    ...
    parameters: {
      ...
      properties: {
        ...
        patch: {
          type: "object",
          description: "要修改的配置项：galleryRoot / scanPaths / thumbnailSize / blogImagesPath / autoClassify",
        },
      },
      required: [],
    },
  ...
  serviceTool({
    ...
    route: "/scan",
    mapArgs: (input) => ({ path: input.path, paths: input.paths }),
    format: (r) => {
      if (r?.ok === false) return fail(r.error);
    ...
    format: (r, input) => {
      ...
      if (action === "list") {
        return callService("/tags/list").then((t) => {
          ...
          return text("标签列表：\n" + t.tags.map((x) => `  ${x.name}（${x.count} 张）`).join("\n"));
        });
      }
  ...
  serviceTool({
    name: "gallery_sync",
    description: "把图库里的图片复制到目标目录（默认配置里的博客图片目录）。只复制不移动，同名文件跳过。建议先用 dry_run 预演。",
    parameters: {
      type: "object",
      properties: {

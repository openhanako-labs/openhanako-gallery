 ...
 * **读不到 app-data 之外的任何文件**。而图库的全部意义就是读用户的图片目录。
 * 平台为此准备了受管运行时：只有 native profile 允许「读当前用户可读的文件」。
  ...
  _starting = (async () => {
    try {
      const rec = await _ctx.runtime.start({
        ...
        profile: "native",
        // native profile 强制要求 network: "external"（宿主校验：
        // "The cross-platform native and local-machine profiles require explicit
        // network: 'external' and its separate grant"），并且需要
        // app/runtime.network 授权。本服务实际只监听 127.0.0.1 供宿主转发，
        // 不发起任何出站请求。
        network: "external",
        cwd: dataDir,
      ...
      logError(`${code}图库服务启动失败: ${detail}`, {
        error: detail,
        hint: "需要 app/runtime.execute + app/runtime.native + app/runtime.network 三项授权；"
          + "native profile 必须配 network: 'external'。",
      });
      return false;
    }
